...
final class CharacterModelAssembler {
  ...
  WorldSceneObjectGeometry assemble(BootstrapAppearance appearance, List<BootstrapItemSlot> equipment) {
    ...
    return geometryByAppearanceKey.computeIfAbsent(cacheKey, ignored -> buildGeometry(appearance, equipment));
  }

  private WorldSceneObjectGeometry buildGeometry(BootstrapAppearance appearance, List<BootstrapItemSlot> equipment) {
    ...
    for (ModelContribution contribution : contributions) {
      ...
      float[][] transformedVertices = transformCharacterVertices(rawModelData, contribution.translateY());
      ...
      for (int faceIndex = 0; faceIndex < rawModelData.faceCount(); faceIndex++) {
        ...
        int recoloredHsl = resolveFaceColor(sourceColorHsl, lookValues, contribution);
  ...
  private float[][] transformCharacterVertices(RawModelData rawModelData, int translateY) {
    ...
    float scale = Math.min(0.82f / Math.max(sourceWidth, sourceDepth), 1.78f / sourceHeight);
    float offsetX = -minX * scale + (0.82f - sourceWidth * scale) / 2.0f - 0.41f;
    float offsetZ = -minZ * scale + (0.82f - sourceDepth * scale) / 2.0f - 0.41f;
    float offsetY = -minY * scale;
    for (int vertexIndex = 0; vertexIndex < vertexCount; vertexIndex++) {
      worldX[vertexIndex] = worldX[vertexIndex] * scale + offsetX;
      worldY[vertexIndex] = worldY[vertexIndex] * scale + offsetY;
      worldZ[vertexIndex] = worldZ[vertexIndex] * scale + offsetZ;
    }
