...
final class WorldSceneSubmissionBuilder {
  ...
  WorldSceneRenderSubmission build(
    ...
    addActorBatches(renderQueueBuilder, worldScene, visibilityWindow, occlusionContext, playerLocalX, playerLocalY, appearance, equipment, actorAnimationState);
  ...
  private void addObjectBatches(
    ...
    for (WorldSceneObject worldSceneObject : worldScene.objects()) {
      ...
      if (worldSceneObject.geometry() != null) {
        ...
        flatBuilder.addGeometry(worldSceneObject.geometry(), objectCenterX, baseHeight, objectCenterZ, SceneRasterMode.FLAT);
        gouraudBuilder.addGeometry(worldSceneObject.geometry(), objectCenterX, baseHeight, objectCenterZ, SceneRasterMode.GOURAUD);
        texturedBuilder.addGeometry(worldSceneObject.geometry(), objectCenterX, baseHeight, objectCenterZ, SceneRasterMode.TEXTURED);
        continue;
      }
  ...
  private void addActorBatches(
    ...
    float baseHeight = worldScene.elevationAt(playerLocalX, playerLocalY) * WorldSceneScale.HEIGHT_SCALE;
    if (WorldSceneOcclusionPlanner.isOccluded(
        occlusionContext,
        playerLocalX + 0.5f,
        baseHeight + 1.1f,
    ...
    if (characterModelAssembler != null) {
      ...
      if (geometry != null) {
        ...
        float actorYawDegrees = actorAnimationState == null ? 0.0f : actorAnimationState.headingDegrees();
        SceneTriangleMeshBuilder flatBuilder = new SceneTriangleMeshBuilder();
        SceneTriangleMeshBuilder gouraudBuilder = new SceneTriangleMeshBuilder();
        SceneTriangleMeshBuilder texturedBuilder = new SceneTriangleMeshBuilder();
  ...
  private float sampleBaseHeight(WorldScene worldScene, float minX, float minZ, float maxX, float maxZ) {
    ...
    for (int x = startX; x <= endX; x++) {
      for (int y = startY; y <= endY; y++) {
        total += worldScene.elevationAt(x, y) * WorldSceneScale.HEIGHT_SCALE;
        samples++;
      }
    }
