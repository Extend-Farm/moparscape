...
final class WorldSceneOccluderBuilder {
  ...
  private float sampleBaseHeight(
    ...
    for (int x = startX; x <= endX; x++) {
      for (int y = startY; y <= endY; y++) {
        total += elevations[y * tileWidth + x] * 0.18f;
        samples++;
      }
    }
  ...
  private GeometryBounds geometryBounds(WorldSceneObject object, float baseHeight) {
    ...
    for (int index = 0; index < object.geometry().vertexX().length; index++) {
      minX = Math.min(minX, object.localX() + object.geometry().vertexX()[index]);
      maxX = Math.max(maxX, object.localX() + object.geometry().vertexX()[index]);
      minY = Math.min(minY, baseHeight + object.geometry().vertexY()[index]);
      maxY = Math.max(maxY, baseHeight + object.geometry().vertexY()[index]);
      minZ = Math.min(minZ, object.localY() + object.geometry().vertexZ()[index]);
      maxZ = Math.max(maxZ, object.localY() + object.geometry().vertexZ()[index]);
    }
