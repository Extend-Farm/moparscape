...
class WorldSceneMinimapRasterizerTest {
  ...
  void rasterizesWallsFromSceneMetadata() {
    ...
    ArgbImage image = rasterizer.rasterize(
        ...
        new byte[16],
        List.of(
            new WorldSceneObject(1, "Stone wall", 1, 1, 0, 0, 0, 1, 1, -1, List.of(), null)
        )
    );
  ...
  void rasterizesShapedTilesUsingLegacyFourByFourMasks() {
    ...
    assertThat(image.pixels()[pixelIndex(4, 0, 0)]).isEqualTo(0xff8b9644);
    assertThat(image.pixels()[pixelIndex(4, 1, 0)]).isEqualTo(0xff5d4721);
  }
  ...
  void stampsMapSceneSpritesInsteadOfGenericAreaObjectFootprints() {
    ...
    ArgbImage image = rasterizer.rasterize(
        ...
        new byte[16],
        List.of(
            new WorldSceneObject(100, "Small house", 1, 1, 0, 10, 0, 1, 1, 0, List.of(), null)
        )
    );
  ...
  void usesWaterTintForLegacyWaterTextureTilesWithoutOverlayRgb() {
    ...
    assertThat(image.pixels()[pixelIndex(4, 1, 1)]).isEqualTo(0xff597ca0);
  }
