...
| Gameplay frame chrome, side tabs, chat shell | `GameClientCore-A.md`, `GameClientCore-D.md`, `widgets.md`, `2d-rendering.md` | `rs-client-lwjgl/.../gameplay/*` and `rs-client-lwjgl/.../gameplay/sidebar/*`, especially `GameplayChromeRenderer` and `sidebar/GameplaySidebarRenderer` | Partial. Inventory, equipment, and stats tabs use real bootstrap data; the combat tab now decodes and renders the cache-backed weapon interface tree; chatbox/status and many other tabs are still explicit placeholders. | Expand widget/interface state and input beyond combat instead of filling more tabs with synthetic content. |
...
2. Widget rendering/input is only partially ported.
   The native path can now decode and render the combat sidebar weapon interfaces, but most widget-driven tabs and overlays still do not exist outside explicit placeholders.
