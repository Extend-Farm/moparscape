...
final class CharacterModelAssembler {
  ...
  private static final float WALK_ARM_SWING_DEGREES = 26.0f;
  private static final float WALK_LEG_SWING_DEGREES = 31.0f;
  private static final float WALK_BODY_BOB = 0.026f;
  private static final float WALK_TORSO_TWIST_DEGREES = 5.8f;
  private static final float WALK_BODY_LEAN_DEGREES = 5.2f;
  private static final float WALK_FOOT_LIFT = 0.072f;
  private static final float WALK_PELVIS_SWAY = 0.018f;
  ...
  WorldSceneObjectGeometry assemble(BootstrapAppearance appearance, List<BootstrapItemSlot> equipment) {
  ...
  WorldSceneObjectGeometry assemble(
  ...
  private PreparedCharacterModel prepareModel(BootstrapAppearance appearance, List<BootstrapItemSlot> equipment) {
    int[] lookValues = normalizedLookValues(appearance);
    ...
    ActorTransform actorTransform = resolveActorTransform(minX, minY, minZ, maxX, maxY, maxZ);
    return new PreparedCharacterModel(
        lookValues,
        preparedContributions,
        actorTransform,
        new ActorBounds(
            minX * actorTransform.scale() + actorTransform.offsetX(),
            minY * actorTransform.scale() + actorTransform.offsetY(),
            minZ * actorTransform.scale() + actorTransform.offsetZ(),
            maxX * actorTransform.scale() + actorTransform.offsetX(),
            maxY * actorTransform.scale() + actorTransform.offsetY(),
            maxZ * actorTransform.scale() + actorTransform.offsetZ()
        )
    );
  }
  ...
  private WorldSceneObjectGeometry buildGeometry(PreparedCharacterModel preparedCharacterModel, ActorAnimationState animationState) {
    ...
    int vertexOffset = 0;
    for (PreparedContribution preparedContribution : preparedCharacterModel.preparedContributions()) {
      ...
      if (!animationState.isIdle()) {
        applyWalkPose(transformedVertices, preparedCharacterModel.actorBounds(), animationState);
      }
  ...
  private int[] normalizedLookValues(BootstrapAppearance appearance) {
  ...
  private void applyWalkPose(
    ...
    float bodyLean = swing * WALK_BODY_LEAN_DEGREES * strideWeight;
    ...
    for (int vertexIndex = 0; vertexIndex < transformedVertices[0].length; vertexIndex++) {
      ...
      if (leanBlend > 0.0f) {
        float[] leaned = rotateAroundZ(x, y, centerX, hipY, bodyLean);
        x = lerp(x, leaned[0], leanBlend * 0.32f);
        y = lerp(y, leaned[1], leanBlend * 0.32f);
  ...
  private ActorTransform resolveActorTransform(
      float minX,
      float minY,
      float minZ,
      float maxX,
    ...
    float sourceWidth = Math.max(0.01f, maxX - minX);
    float sourceDepth = Math.max(0.01f, maxZ - minZ);
    float sourceHeight = Math.max(0.01f, maxY - minY);
    float scale = Math.min(ACTOR_FOOTPRINT / Math.max(sourceWidth, sourceDepth), ACTOR_HEIGHT / sourceHeight);
    float offsetX = -minX * scale + (ACTOR_FOOTPRINT - sourceWidth * scale) / 2.0f - (ACTOR_FOOTPRINT * 0.5f);
    float offsetZ = -minZ * scale + (ACTOR_FOOTPRINT - sourceDepth * scale) / 2.0f - (ACTOR_FOOTPRINT * 0.5f);
    float offsetY = -minY * scale;
    return new ActorTransform(scale, offsetX, offsetY, offsetZ);
  }

  private float[][] applyActorTransform(PreparedContribution contribution, ActorTransform actorTransform) {
  ...
  private int resolveFaceColor(int faceColor, int[] lookValues, ModelContribution contribution) {
  ...
  private float lerp(float start, float end, float weight) {
  ...
  private float[] rotateAroundX(float y, float z, float pivotY, float pivotZ, float degrees) {
  ...
  private float[] rotateAroundY(float x, float z, float pivotX, float pivotZ, float degrees) {
    float radians = (float) Math.toRadians(degrees);
    float cosine = (float) Math.cos(radians);
    float sine = (float) Math.sin(radians);
  ...
  private float[] rotateAroundZ(float x, float y, float pivotX, float pivotY, float degrees) {
    float radians = (float) Math.toRadians(degrees);
    float cosine = (float) Math.cos(radians);
    float sine = (float) Math.sin(radians);
    float shiftedX = x - pivotX;
  ...
  private record PreparedContribution(
  ...
  private record ActorBounds(float minX, float minY, float minZ, float maxX, float maxY, float maxZ) {
