...
final class OpenGlSceneRasterBackend implements SceneRasterBackend, AutoCloseable {
  ...
  OpenGlSceneRasterBackend(SceneTextureAssets sceneTextureAssets) {
  ...
  private void drawTexturedMesh(SceneTriangleMesh mesh, int[] faceIndices) {
    ...
    for (int faceIndex : faceIndices) {
      OpenGlTexture texture = resolveTexture(mesh.faceTextureIds()[faceIndex]);
      float[] uv = texture == null ? null : computeTextureCoordinates(mesh, faceIndex);
  ...
  private float[] computeTextureCoordinates(SceneTriangleMesh mesh, int faceIndex) {
    ...
    float[] uvA = computeUv(anchorA, anchorB, anchorC, vertex(mesh, mesh.faceVertexA()[faceIndex]));
    float[] uvB = computeUv(anchorA, anchorB, anchorC, vertex(mesh, mesh.faceVertexB()[faceIndex]));
    float[] uvC = computeUv(anchorA, anchorB, anchorC, vertex(mesh, mesh.faceVertexC()[faceIndex]));
  ...
  private float[] computeUv(float[] anchorA, float[] anchorB, float[] anchorC, float[] point) {
