...
import com.veyrmoor.client.desktop.world.raster.SceneRenderBatch;
...
class WorldSceneSubmissionBuilderTest {
  ...
  @Test
  void reusesTerrainMeshesAcrossBuildsForTheSameWorldScene() {
    WorldScene worldScene = testWorldScene();
    WorldSceneSubmissionBuilder submissionBuilder = new WorldSceneSubmissionBuilder(null);

    WorldSceneRenderSubmission firstSubmission = submissionBuilder.build(
        worldScene,
        new WorldPoint(3204, 3204, 0),
        null,
        List.of(),
    ...
    WorldSceneRenderSubmission secondSubmission = submissionBuilder.build(
        worldScene,
        new WorldPoint(3204, 3204, 0),
        null,
        List.of(),
    ...
    assertThat(batchOf(firstSubmission, SceneSubmissionKind.TILE_PAINT, SceneRasterMode.TEXTURED).mesh())
        .isSameAs(batchOf(secondSubmission, SceneSubmissionKind.TILE_PAINT, SceneRasterMode.TEXTURED).mesh());
    assertThat(batchOf(firstSubmission, SceneSubmissionKind.TILE_MODEL, SceneRasterMode.GOURAUD).mesh())
        .isSameAs(batchOf(secondSubmission, SceneSubmissionKind.TILE_MODEL, SceneRasterMode.GOURAUD).mesh());
    assertThat(batchOf(firstSubmission, SceneSubmissionKind.TILE_MODEL, SceneRasterMode.TEXTURED).mesh())
        .isSameAs(batchOf(secondSubmission, SceneSubmissionKind.TILE_MODEL, SceneRasterMode.TEXTURED).mesh());
  }
