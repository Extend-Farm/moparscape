...
public final class CacheBackedWorldSceneLoader {
  ...
  private final WorldSceneMinimapRasterizer minimapRasterizer = new WorldSceneMinimapRasterizer();
  ...
  public CacheBackedWorldSceneLoader(Path workingDirectory) {
    ...
    this.objectAssembler = new WorldSceneObjectAssembler(
        objectDefinitions,
        new ObjectSceneGeometryBuilder(new RawModelRepository(manifest.cacheStore()))
    );
  }
  ...
  public WorldScene load(WorldPoint worldPoint) {
    ...
    int[] tileElevations = new int[SCENE_TILE_SIZE * SCENE_TILE_SIZE];
    ...
    ArgbImage minimapImage = renderMinimap(tileColors, sceneObjects);
  ...
  private void captureRegion(
    ...
    for (int tileX = 0; tileX < REGION_SIZE; tileX++) {
      for (int tileY = 0; tileY < REGION_SIZE; tileY++) {
        ...
        int effectivePlane = WorldScenePlaneRules.effectivePlane(regionData, plane, tileX, tileY);
        int sceneIndex = sceneY * SCENE_TILE_SIZE + sceneX;
        underlayColors[sceneIndex] = tileUnderlayColor(regionData, effectivePlane, tileX, tileY);
        overlayColors[sceneIndex] = tileOverlayColor(regionData, effectivePlane, tileX, tileY);
        underlayTextureIds[sceneIndex] = tileUnderlayTextureId(regionData, effectivePlane, tileX, tileY);
        overlayTextureIds[sceneIndex] = tileOverlayTextureId(regionData, effectivePlane, tileX, tileY);
        tileColors[sceneIndex] = tileActiveColor(underlayColors[sceneIndex], overlayColors[sceneIndex]);
        overlayShapes[sceneIndex] = (byte) regionData.overlayShapeAt(effectivePlane, tileX, tileY);
        overlayRotations[sceneIndex] = (byte) regionData.overlayRotationAt(effectivePlane, tileX, tileY);
        tileFlags[sceneIndex] = (byte) regionData.tileFlagAt(plane, tileX, tileY);
        tileElevations[sceneIndex] = tileElevation(regionData, effectivePlane, tileX, tileY);
      }
    }
  }
  ...
  private void captureObjects(
    ...
    for (MapObjectPlacement placement : objectRegionData.placements()) {
      ...
      int effectivePlane = WorldScenePlaneRules.effectivePlane(
          terrainRegionData,
          placement.plane(),
          regionTileX,
          regionTileY
      );
      if (effectivePlane != plane) {
        continue;
      }
  ...
  private int tileUnderlayColor(TerrainRegionData regionData, int plane, int tileX, int tileY) {
    return shadeFloorColor(floorColors.colorFor(regionData.underlayIdAt(plane, tileX, tileY)), regionData, plane, tileX, tileY);
  }

  private int tileOverlayColor(TerrainRegionData regionData, int plane, int tileX, int tileY) {
    return shadeFloorColor(floorColors.colorFor(regionData.overlayIdAt(plane, tileX, tileY)), regionData, plane, tileX, tileY);
  }
  ...
  private int shadeFloorColor(int rgb, TerrainRegionData regionData, int plane, int tileX, int tileY) {
    if (rgb == 0) {
      return 0;
    }
    int westX = Math.max(tileX - 1, 0);
    int eastX = Math.min(tileX + 1, REGION_SIZE - 1);
    int southY = Math.max(tileY - 1, 0);
    int northY = Math.min(tileY + 1, REGION_SIZE - 1);
    int xGradient = regionData.heightAt(plane, eastX, tileY) - regionData.heightAt(plane, westX, tileY);
    int yGradient = regionData.heightAt(plane, tileX, northY) - regionData.heightAt(plane, tileX, southY);
    // Terrain colors are cheap-shaded from the local height gradient so the native client can show
    // readable relief before the full legacy-equivalent scene graph and lighting pipeline exists.
    int brightness = clamp(128 - xGradient / 8 - yGradient / 8, 72, 180);
  ...
  private ArgbImage renderMinimap(int[] tileColors, List<WorldSceneObject> sceneObjects) {
    // The minimap now rasterizes directly from the same scene object metadata that feeds the main
    // viewport instead of only painting generic object footprints over terrain colors.
    return minimapRasterizer.rasterize(
        SCENE_TILE_SIZE,
        SCENE_TILE_SIZE,
        tileColors,
        underlayColorsForMinimap(tileColors.length),
        overlayColorsForMinimap(tileColors.length),
        overlayShapesForMinimap(tileColors.length),
        overlayRotationsForMinimap(tileColors.length),
        sceneObjects
    );
  }

  private int[] underlayColorsForMinimap(int expectedSize) {
    throw new UnsupportedOperationException("placeholder");
  }

  private int[] overlayColorsForMinimap(int expectedSize) {
    throw new UnsupportedOperationException("placeholder");
  }

  private byte[] overlayShapesForMinimap(int expectedSize) {
    throw new UnsupportedOperationException("placeholder");
  }

  private byte[] overlayRotationsForMinimap(int expectedSize) {
    throw new UnsupportedOperationException("placeholder");
  }
