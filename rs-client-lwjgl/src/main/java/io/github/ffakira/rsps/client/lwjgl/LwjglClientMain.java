...
public final class LwjglClientMain {
  ...
  public static void main(String[] args) {
    ...
    try {
      LwjglWindowConfig windowConfig = LwjglWindowConfig.defaults();
  ...
  private static void bindWindowCallbacks(
    ...
    glfwSetMouseButtonCallback(window, (windowHandle, button, action, mods) -> {
      if (NativeClientRuntimeCoordinator.isGameplayActive(session.viewModel())) {
        if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
          renderSystem.handleGameplayClick(mouseX[0], mouseY[0]);
        }
        return;
      }
      if (button != GLFW_MOUSE_BUTTON_LEFT || action != GLFW_PRESS) {
    ...
    glfwSetKeyCallback(window, (windowHandle, key, scancode, action, mods) -> {
      if (NativeClientRuntimeCoordinator.isGameplayActive(session.viewModel())) {
        handleGameplayKey(windowHandle, key, action, session);
        return;
      }
  ...
  private static void handleGameplayKey(
    ...
    switch (key) {
      case GLFW_KEY_LEFT, GLFW_KEY_A -> gameplayClientSession.move(-1, 0, MovementMode.WALK);
      case GLFW_KEY_RIGHT, GLFW_KEY_D -> gameplayClientSession.move(1, 0, MovementMode.WALK);
      case GLFW_KEY_UP, GLFW_KEY_W -> gameplayClientSession.move(0, 1, MovementMode.WALK);
      case GLFW_KEY_DOWN, GLFW_KEY_S -> gameplayClientSession.move(0, -1, MovementMode.WALK);
      default -> {
      }
    }
  }
