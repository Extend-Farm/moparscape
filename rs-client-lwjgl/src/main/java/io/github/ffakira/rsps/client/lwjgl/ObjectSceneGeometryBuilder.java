...
final class ObjectSceneGeometryBuilder {
  ...
  WorldSceneObjectGeometry build(
    ...
    for (Integer modelId : modelIds) {
      ...
      float[][] transformedVertices = transformVertices(rawModelData, definition, orientation, sizeX, sizeY);
      ...
      for (int faceIndex = 0; faceIndex < rawModelData.faceCount(); faceIndex++) {
        ...
        if (faceMode == 0 || faceMode == 2) {
          faceColorA.add(shadeObjectColor(
              baseRgb,
          ...
          faceColorB.add(shadeObjectColor(
              baseRgb,
          ...
          faceColorC.add(shadeObjectColor(
              baseRgb,
          ...
          int shadedFaceColor = shadeObjectColor(
              baseRgb,
  ...
  private float[][] transformVertices(
    ...
    float scaleToFootprint = Math.min(sizeX / sourceWidth, sizeY / sourceDepth);
    float offsetX = (sizeX - sourceWidth * scaleToFootprint) / 2.0f;
    float offsetZ = (sizeY - sourceDepth * scaleToFootprint) / 2.0f;
    for (int vertexIndex = 0; vertexIndex < vertexCount; vertexIndex++) {
      worldX[vertexIndex] = (worldX[vertexIndex] - minX) * scaleToFootprint + offsetX;
      worldY[vertexIndex] = worldY[vertexIndex] - minY;
      worldZ[vertexIndex] = (worldZ[vertexIndex] - minZ) * scaleToFootprint + offsetZ;
    }
