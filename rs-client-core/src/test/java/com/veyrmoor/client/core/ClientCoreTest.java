...
class ClientCoreTest {
  ...
  @Test
  void appendsPublicChatMessagesToTheViewModelHistory() {
    ClientCore clientCore = new ClientCore();

    clientCore.accept(new PublicChatMessage("Akira", "Selling lobster"));

    assertThat(clientCore.viewModel().chatMessages()).containsExactly(
        ClientChatMessage.publicChat("Akira", "Selling lobster")
    );
    assertThat(clientCore.viewModel().chatMessages().getFirst().formattedText()).isEqualTo("Akira: Selling lobster");
  }
}
