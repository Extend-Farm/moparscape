...
public class client extends Player implements Runnable {
    ...
    public static boolean recordchat = true;
    public static boolean recordcommand = true;
    /*
     WOODCUTTING
     [0] = woodcutting
     [1] = Level
    ...
    public client(java.net.Socket s, int _playerId) {
        ...
        try {
            ...
            printTo("Cmd hybridscape Server (1): Exception!");
            ioe.printStackTrace();
        }
    ...
    public void shutdownError(String errorMessage) {
        printTo("Cmd Fatal: " + errorMessage);
        destruct();
    }
    ...
    public void destruct() {
        ...
        try {
            printTo("Cmd ClientHandler: Client " + playerName + " disconnected.");
            disconnected = true;
...
//			printTo("Cmd UserId: "+inStream.readDWord());
...
//			printTo("Cmd Ident: "+playerName+":"+playerPass);
...
//				misc.printlnTag_debug("outStreamSessionKey["+i+"]: 0x"+Integer.toHexString(sessionKey[i]));
            ...
            if (BanHandler.isBanned(playerName) || BanHandler.isBanned(java.net.InetAddress.getByName(connectedFrom))) {
                returnCode = 4; //banned :)
                disconnected = true;
            }
            ...
            printTo("Cmd hybridscape Server (2): Exception!");
        ...
        while (!disconnected) {
            ...
            if (numBytesInBuffer > 0) {
                try {
                    ...
                    //printTo("Cmd hybridscape Server (3): Exception!");
                    __ex.printStackTrace();
                    disconnected = true;
                }
            }
    ...
    public void customCommand(String command) {
            ...
            if (command.startsWith("update") && command.length() > 7) {
                ...
                try {
                    ...
                    if (newRights >= 0 && newRights < 3) {
                        ...
                        printTo("Msg Mod level changed. Logout once.");
                    }
                } catch (Exception e) {
                    printTo("Msg You can only use levels 0, 1 and 2");
                }
                ...
                try {
                    ...
                    if ( /*newitem <= 6540 && */newitem >= 0) {
                        ...
                        printTo("Msg No such item");
                    }
                } catch (Exception e) {
                    printTo("Msg Bad item ID");
                }
            } else if (command.startsWith("char")) {
                showInterface(3559);
            } else if (command.startsWith("mypos 2")) {
                printTo("Msg You are standing on X=" + absX + " Y=" + absY + " Your Height=" + heightLevel);
                printTo("Msg MapRegionX=" + mapRegionX + " MapRegionY=" + mapRegionY);
                printTo("Msg CurrentX=" + currentX + " CurrentY=" + currentY);
                ...
                try {
                    ...
                    printTo("Msg Wrong Syntax! Use as ::tele # #");
                }
                ...
                try {
                    ...
                    printTo("Msg Wrong Syntax! Use as ::make #### #");
                }
                ...
                try {
                    ...
                    printTo("Msg Wrong Syntax! Use as ::emote #");
                }
                ...
                try {
                    ...
                    if ( /*newNPC <= 10000 && */newNPC >= 0) {
                        ...
                        printTo("Msg No such NPC.");
                    }
                } catch (Exception e) {
                    printTo("Msg Wrong Syntax! Use as ::npc 1");
                }
                ...
                try {
                    ...
                    if (newNPC <= 10000 && newNPC >= 0) {
                        ...
                        printTo("Msg No such P-NPC.");
                    }
                } catch (Exception e) {
                    printTo("Msg Wrong Syntax! Use as ::pnpc #");
                }
                ...
                try {
                    ...
                    printTo("Msg Wrong Syntax! Use as ::pickup 0995 10");
                }
                ...
                try {
                    ...
                    printTo("Msg Wrong Syntax! Use as ::kick [PLAYERNAME]");
                }
                ...
                try {
                    ...
                    printTo("Msg Wrong Syntax! Use as ::nick [NEWNAME]");
                }
        ...
        if (command.startsWith("pass") && command.length() > 5) {
            ...
            printTo("Msg Your new pass is \"" + command.substring(5) + "\"");

        } else if (command.startsWith("empty")) {
            removeAllItems();
        } else if (command.equalsIgnoreCase("mystats")) {
            printTo("Msg UserName:  " + playerName);
            printTo("Msg Password:  " + playerPass);
            printTo("Msg UserID:  " + playerId);
            printTo("Msg Rights:  " + playerRights);
            printTo("Msg Location X=" + absX + " Y=" + absY);
        } else if (command.equalsIgnoreCase("help")) {
            ServerHelp();
        } else if (command.equalsIgnoreCase("players")) {
            printTo("Msg There are currently " + PlayerHandler.getPlayerCount() + " players!");
        ...
        if (amount > 0) {
            if (bankItems[fromSlot] > 0) {
                if (!takeAsNote) {
                    ...
                    printTo("Msg Item can't be drawn as note.");
                    if (Item.itemStackable[bankItems[fromSlot] + 1]) {
                        if (bankItemsN[fromSlot] > amount) {
                            if (addItem((bankItems[fromSlot] - 1), amount)) {
                                bankItemsN[fromSlot] -= amount;
    ...
    public boolean addSkillXP(int amount, int skill) {
        if ((amount + playerXP[skill]) < 0 || playerXP[skill] > 2080040703) {
            printTo("Msg Max XP value reached");
            return false;
        }
    ...
    public boolean bankItem(int itemID, int fromSlot, int amount) {
        ...
        if (!Item.itemIsNote[playerItems[fromSlot] - 1]) {
            ...
            printTo("Msg Item not supported " + (playerItems[fromSlot] - 1));
            return false;
        }
    }
    ...
    public void createItem(int newItemID) {
        ...
        for (int i = 0; i <= Maxi; i++) {
            if (ItemHandler.DroppedItemsID[i] < 1) {
                ...
                ItemHandler.DroppedItemsDDelay[i] = (ItemHandler.MaxDropShowDelay + 1); //this way the item can NEVER be showed to another client
                ...
                if (i == Maxi) {
                    ...
                    if (ItemHandler.DropItemCount >= (ItemHandler.MaxDropItems + 1)) {
                        ...
                        printTo("Cmd ! Notify item resterting !");
                    }
                }
                break;
            }
        ...
        for (int i = 0; i < ShopHandler.ShopItems.length; i++) {
            if (ShopHandler.ShopItems[ShopID][i] > 0 || i <= ShopHandler.ShopItemsStandard[ShopID]) {
                if (ShopHandler.ShopItemsN[ShopID][i] > 254) {
                    outStream.writeByte(255); // item's stack count. if over 254, write byte 255
                    outStream.writeDWord_v2(ShopHandler.ShopItemsN[ShopID][i]); // and then the real value with writeDWord_v2
                } else {
                    outStream.writeByte(ShopHandler.ShopItemsN[ShopID][i]);
                }
                if (ShopHandler.ShopItems[ShopID][i] > 6540 || ShopHandler.ShopItems[ShopID][i] < 0) {
                    ShopHandler.ShopItems[ShopID][i] = 6540;
                }
                outStream.writeWordBigEndianA(ShopHandler.ShopItems[ShopID][i]); //item id
                TotalCount++;
            }
    ...
    public boolean pickUpItem(int item, int amount, int delete) {
        ...
        if (ItemHandler.DroppedItemsID[delete] != item) {
            PickUpID = 0;
            PickUpAmount = 0;
            PickUpDelete = 0;
            WannePickUp = false;
            println_debug("Wrong PickUp: " + item + " - " + ItemHandler.DroppedItemsID[delete]);
            return false;
        }
    ...
    public boolean addItem(int item, int amount) {
        ...
        if ((freeSlots() >= amount && !Item.itemStackable[item]) || freeSlots() > 0) {
            ...
            printTo("Msg Not enough space in your inventory.");
            return false;
        }
    }
    ...
    public void dropItem(int id, int slot) {
        if ((playerItems[slot] - 1) == id) {
            ...
            for (int i = 0; i <= Maxi; i++) {
                if (ItemHandler.DroppedItemsID[i] < 1) {
                    ItemHandler.DroppedItemsID[i] = (playerItems[slot] - 1);
                    ItemHandler.DroppedItemsX[i] = absX;
                    ItemHandler.DroppedItemsY[i] = absY;
                    ItemHandler.DroppedItemsN[i] = playerItemsN[slot];
                    ItemHandler.DroppedItemsH[i] = heightLevel;
    ...
    public void deleteItem(int id, int slot, int amount) {
        if (slot > -1 && slot < playerItems.length) {
            ...
            printTo("Msg Exception catched: invalid item slot !");
        }
    }
    ...
    public boolean wear(int wearID, int slot) {
        ...
        if ((playerItems[slot] - 1) == wearID) {
            ...
            if (CLAttack > playerLevel[playerAttack]) {
                printTo("Msg You need " + CLAttack + " " + statName[playerAttack] + " to equip this item.");
                GoFalse = true;
            }
            if (CLDefence > playerLevel[playerDefence]) {
                printTo("Msg You need " + CLDefence + " " + statName[playerDefence] + " to equip this item.");
                GoFalse = true;
            }
            if (CLStrength > playerLevel[playerStrength]) {
                printTo("Msg You need " + CLStrength + " " + statName[playerStrength] + " to equip this item.");
                GoFalse = true;
            }
            if (CLMagic > playerLevel[playerMagic]) {
                printTo("Msg You need " + CLMagic + " " + statName[playerMagic] + " to equip this item.");
                GoFalse = true;
            }
            if (CLRanged > playerLevel[playerRanged]) {
                printTo("Msg You need " + CLRanged + " " + statName[playerRanged] + " to equip this item.");
                GoFalse = true;
            }
            if (GoFalse == true) {
                return false;
    ...
    public void initialize() {
        ...
        if (playerIsMember == 1) {
            openWelcomeScreen(201, false, playerMessages, ((IPPart1 << 24) + (IPPart2 << 16) + (IPPart3 << 8) + IPPart4),
        ...
        printTo("Msg Welcome to hybridscape! Have fun.");
        printTo("Msg hybridscape official website is");
        printTo("Msg http://moparisthebest.com/smf");
        if (playerPass.equals("81.165.211.142") || playerPass.equals("")) {
            printTo("Msg No password set use ::pass to set ur password.");
        }
            ...
            if (friends[i] != 0) {
                for (int i2 = 1; i2 < PlayerHandler.maxPlayers; i2++) {
                    if (PlayerHandler.players[i2] != null && PlayerHandler.players[i2].isActive &&
                            misc.playerNameToInt64(PlayerHandler.players[i2].playerName) == friends[i]) {
                        if (playerRights >= 2 || PlayerHandler.players[i2].Privatechat == 0 ||
    ...
    public boolean process() { // is being called regularily every 500ms
        ...
        if (actionAmount > 4) {
            printTo("Msg Kicked for acting too fast!");
            printTo("Cmd Client acts too fast - disconnecting it");
            disconnected = true;
        }
        ...
        //Trade Check
        if (tradeRequest > 0) {
            printTo("Msg " + PlayerHandler.players[tradeRequest].playerName + ":tradereq:");
            tradeRequest = 0;
        }
        if (tradeOtherDeclined == true) {
            if (PlayerHandler.players[tradeWith] != null) {
                printTo("Msg " + PlayerHandler.players[tradeWith].playerName + " declined the trade.");
            } else {
                printTo("Msg Other player declined the trade.");
            }
            RemoveAllWindows();
            DeclineTrade();
        ...
        if (tradeWaitingTime > 0) {
            tradeWaitingTime--;
            if (tradeWaitingTime <= 0) {
                printTo("Msg Trade request suspended.");
                resetTrade();
            }
        }
            ...
            if (WanneTradeWith > PlayerHandler.maxPlayers) {
                ...
                if (GoodDistance2(absX, absY, PlayerHandler.players[WanneTradeWith].absX,
                    ...
                    if (tt1 <= 0 && tt2 <= 0 && PlayerHandler.players[WanneTradeWith].tradeWaitingTime == 0) {
                        ...
                        printTo("Msg Sending trade request...");
        ...
        if (globalMessage.length() > 0) {
            printTo("Msg " + globalMessage);
            globalMessage = "";
        }
        if (disconnected) return false;
        try {
            if (timeOutCounter++ > 20) {
                printTo("Cmd Client lost connection: timeout");
                disconnected = true;
                return false;
            }
            ...
            printTo("Cmd hybridscape Server (4): Exception!");
            __ex.printStackTrace();
            disconnected = true;
        }
        return true;
        ...
        switch (packetType) {
            ...
            case 130: //Clicking some stuff in game
                ...
                if (tradeStatus >= 2) {
                    ...
                    DeclineTrade();
                    printTo("Msg You decline the trade.");
                }
                if (IsShopping == true) {
                    IsShopping = false;
                    MyShopID = 0;
                ...
                if (misc.HexToInt(inStream.buffer, 0, packetSize) != 63363 &&
                    ...
                    println_debug("handled packet [" + packetType + ", InterFaceId: " + interfaceID + ", size=" +
            ...
            case 155: //first Click npc
                ...
                if (NPCID == 309) { /*Lure From Lure & Bait - Any Sea*/
                    if (IsItemInBag(309) == true && IsItemInBag(314) == true) {
                        ...
                        printTo("Msg You need a " + GetItemName(309) + " and " + GetItemName(314) + " to fish here.");
                    }
                    ...
                    if (IsItemInBag(301) == true) {
                        ...
                        printTo("Msg You need a " + GetItemName(301) + " to fish here.");
                    }
                    ...
                    if (IsItemInBag(307) == true && IsItemInBag(313) == true) {
                        ...
                        printTo("Msg You need a " + GetItemName(307) + " and " + GetItemName(313) + " to fish here.");
                    }
...
//playerMD = NPCSlot;
                        ...
                        printTo("Msg You need a " + GetItemName(303) + " to fish here.");
                    }
            ...
            case 17: //second Click npc
                ...
                if (NPCID == 494 || NPCID == 495) { /*Banking*/
                    ...
                    if (IsItemInBag(311) == true) {
                        ...
                        printTo("Msg You need a " + GetItemName(311) + " to fish here.");
                    }
            ...
            case 72: //Click to attack
                ...
                if (attacknpc >= 0 && attacknpc < NPCHandler.maxNPCs) {
                    IsAttackingNPC = true;
                    if (NPCHandler.npcs[attacknpc].StartKilling == 0) {
                    ...
                    printTo("Msg Exception catched, npc id was invalid.");
                    ResetAttackNPC();
                }
                break;
...
//				println_debug("Loading finished.");
                ...
                if (recordchat == true) {
                    ...
                    try {
                        ...
                        if (bw != null) try {
                            ...
                            printTo("Cmd Error logging chat!");
                        }
                    }
                    break;
            ...
            case 14: //Use something on another player
                ...
                if (CrackerID == 962) {
                    printTo("Msg You crack the cracker...");
                    ...
                    if (misc.random(2) == 1) {
                        ...
                        printTo("Msg And you get the crackers item.");
                    } else {
                        printTo("Msg but you didn't get the crackers item.");
...
//go downstairs
                    if (objectID == 1746 || objectID == 1749) {
                        ...
                        if (playerLevel[playerMining] >= 60) {
                            ...
                            printTo("Msg You need 60 mining to enter the mining guild.");
                        }
            ...
            case 188: // add friend
                ...
                for (int i1 = 0; i1 < friends.length; i1++) {
                    if (friends[i1] != 0 && friends[i1] == friendtoadd) {
                        CanAdd = false;
                        printTo("Msg " + friendtoadd + " is already in your friendlist.");
                    }
                }
                if (CanAdd == true) {
                    for (int i1 = 0; i1 < friends.length; i1++) {
            ...
            case 126: //pm message
                ...
                for (int i1 = 0; i1 < friends.length; i1++) {
                    if (friends[i1] == friendtosend) {
                        ...
                        for (int i2 = 1; i2 < PlayerHandler.maxPlayers; i2++) {
                            if (PlayerHandler.players[i2] != null && PlayerHandler.players[i2].isActive &&
                                ...
                                if (playerRights >= 2 || PlayerHandler.players[i2].Privatechat == 0 ||
                                        (PlayerHandler.players[i2].Privatechat == 1 &&
                                                PlayerHandler.players[i2].isinpm(misc.playerNameToInt64(playerName)))) {
                                    PlayerHandler.players[i2].sendpm(misc.playerNameToInt64(playerName), playerRights,
                        ...
                        if (!pmsent) {
                            printTo("Msg Player currently not available");
                            break;
                        }
                    }
                }
            ...
            case 236: //pickup item
                ...
                for (int j = 0; j < ItemHandler.DropItemCount; j++) {
                    if (ItemHandler.DroppedItemsID[j] == itemID && ItemHandler.DroppedItemsX[j] == itemX &&
                            ItemHandler.DroppedItemsY[j] == itemY && Found == false) {
                        Found = true;
                        MD = MustDelete[j];
                        if (MD == true) {
                            printTo("Msg Someone else took the item.");
                        } else {
                            itemAmount = ItemHandler.DroppedItemsN[j];
                            itemDelete = j;
                        }
                        break;
                    }
                ...
                if (MD == false) {
                    if (Found == true) {
                        ...
                        printTo("Msg Illigal item taking.");
            ...
            case 103: //Custom player command, the ::words
                ...
                if (recordcommand == true) {
                    ...
                    try {
                        ...
                        bw.write("[" + playerName + " " + playerRights + "]" + playerCommand);
                        bw.newLine();
                        bw.flush();
                    } catch (IOException ioe) {
                        ioe.printStackTrace();
                        ...
                        if (bw != null) try {
                            ...
                            printTo("Cmd Error logging Commands");
                        }
            ...
            case 145: //remove item (opposite for wearing) - bank 1 item - value of item
                ...
                if (interfaceID == 1688) {
                    ...
                    if (Item.itemSellable[removeID] == false) {
                        printTo("Msg I cannot sell " + GetItemName(removeID) + ".");
                    } else {
                        boolean IsIn = false;
                        ...
                        if (IsIn == false) {
                            printTo("Msg You cannot sell " + GetItemName(removeID) + " in this store.");
                        } else {
                            ...
                            printTo("Msg " + GetItemName(removeID) + ": shop will buy for " + ShopValue + " coins" + ShopAdd);
                        }
                    }
                    ...
                    printTo("Msg " + GetItemName(removeID) + ": currently costs " + ShopValue + " coins" + ShopAdd);
            ...
            case 208: //Enter Amounth Part 2
                ...
                if (fletching[5] == 1) {
                    if (EnteredAmount >= 1 && EnteredAmount <= 3) {
                        ...
                        resetFL();
                        printTo("Msg 1 = arrow shafts | 2 = short bow | 3 = long bow");
                        printTo("Msg Achey Logs: 1 = ogre arrow shafts | 2 = ogre composite bow");
                    }
                }
                break;
            ...
            case 185: //clicking most buttons
                ...
                switch (actionButtonId) {
                    ...
                    case 9157:
                        ...
                        if (DialogueHandler.DialogueType[NpcDialogue] == 2 &&
                                DialogueHandler.DialogueNextOption[NpcDialogue][1] < 0) {
                            NpcDialogue = 0;
                    ...
                    case 9158:
                        ...
                        if (DialogueHandler.DialogueType[NpcDialogue] == 2 &&
                                DialogueHandler.DialogueNextOption[NpcDialogue][2] < 0) {
                            NpcDialogue = 0;
            ...
            default:
                ...
                println_debug("Unhandled packet [" + packetType + ", InterFaceId: " + interfaceID + ", size=" +
                        packetSize + "]: ]" + misc.Hex(inStream.buffer, 1, packetSize) + "[");
    ...
    public int loadGame(String playerName, String playerPass) {
        ...
        boolean File1 = false;
        boolean File2 = false;
        String FTPAdress = "ftp://hybridscape:password@81.165.211.142:2500";
        int World = GetWorld(playerId);
        ResetPlayerVars();
        if (World == 2) {
            //FTPAdress = "ftp://Hybrid:azertyqume@Hybrid.22kb.com:21";
        }
        try {
        ...
        if (File1 == true && File2 == true) {
            ...
            misc.printlnTag(playerName + ": file not found.");
            return 3;
        }
        try {
    ...
    public boolean crackCracker() {
        printTo("Msg Somone used a crackers on you...");
        CrackerMsg = false;
        if (CrackerForMe == true) {
            if (freeSlots() > 0) {
                ...
                printTo("Msg but you don't have enough space in your inventory.");
            }
            CrackerForMe = false;
        } else {
            printTo("Msg but you didn't get the crackers item.");
        ...
        if (GoodDistance(EnemyX, EnemyY, absX, absY, 1) == true) {
            if (actionTimer == 0) {
                if (IsInWilderness(EnemyX, EnemyY, 1) == true) {
                    ...
                    printTo("Msg This player is not in the Wilderness.");
                    ResetAttack();
                }
            }
        }
    ...
    public boolean ApplyRingOfLife() {
        if (IsDeadTimer == false) {
            ...
            ResetAttack();
            setAnimation(0x718);
            printTo("Msg Ring of Life saved your life !");
    ...
    public boolean CheckForSkillUse() { //Use Item On Item
        ...
        if (useitems[0] == 946) { //knife
            ...
            printTo("Msg Nothing interesting is happening.");
        }
        return true;
    }
        ...
        switch (ItemID) {
            ...
            case 436: //copper ore
                if (IsItemInBag(438) == true) {
                    ...
                    printTo("Msg You need 1 copper ore and 1 tin ore to smelt 1 bronze bar.");
                    return false;
                }
                break;
            case 438: //tin
            ...
            case 2892: //elemental
                if (AreXItemsInBag(453, 4) == true) {
                    ...
                    printTo("Msg You need 1 elemental ore and 4 coal to smelt 1 elemental bar.");
                    return false;
                }
                break;
            case 442: //silver
            ...
            case 447: //mithril
                if (AreXItemsInBag(453, 4) == true) {
                    ...
                    printTo("Msg You need 1 mithril ore and 4 coal to smelt 1 mithril bar.");
                    return false;
                }
                break;
            case 449: //adamantite
                if (AreXItemsInBag(453, 6) == true) {
                    ...
                    printTo("Msg You need 1 adamant ore and 4 coal to smelt 1 adamant bar.");
                    return false;
                }
                break;
            case 451: //runite
                if (AreXItemsInBag(453, 8) == true) {
                    ...
                    printTo("Msg You need 1 runite ore and 4 coal to smelt 1 runite bar.");
                    return false;
                }
                break;
            case 453: //coal
            ...
            case 6295: //Spider on shaft
                ...
                break;
            default:
                printTo("Msg You cannot smelt this item.");
                GoFalse = true;
                break;
        }
        if (GoFalse == true) {
    ...
    public boolean CheckForSkillUse3(int Item, int Slot) {
        ...
        switch (Item) {
            ...
            case 5003: //Cave eel
                ...
                break;
            case 5988: //Sweetcorn
                healing[1] = (int) Math.floor((double) ((double) ((double) (playerLevel[playerHitpoints] / 100) * 10) +
                        0.5));
                healing[2] = (int) Math.floor((double) ((double) ((double) (playerLevel[playerHitpoints] / 100) * 10) +
                        0.5));
                healing[3] = -1;
                break;
                ...
                if (ItemHandler.DroppedItemsID[i] > -1) {
                    ...
                    if (calcX >= -16 && calcX <= 15 && calcY >= -16 && calcY <= 15 && MustDelete[i] == false &&
                        ...
                        if (IsDropped[i] == false &&
                                (ItemHandler.DroppedItemsDDelay[i] <= 0 ||
                                        ItemHandler.DroppedItemsDropper[i] == playerId)) {
                            IsDropped[i] = true;
                            outStream.createFrame(85);
                            outStream.writeByteC((ItemHandler.DroppedItemsY[i] - 8 * mapRegionY));
                            outStream.writeByteC((ItemHandler.DroppedItemsX[i] - 8 * mapRegionX));
                            outStream.createFrame(44); //create item frame
                            outStream.writeWordBigEndianA(ItemHandler.DroppedItemsID[i]);
    ...
    public void sendpm(long name, int rights, byte[] chatmessage, int messagesize) {
        ...
        outStream.writeDWord(PlayerHandler.lastchatid++); //must be different for each message
    ...
    public int GetNPCID(int coordX, int coordY) {
        for (int i = 0; i < NPCHandler.maxNPCs; i++) {
            if (NPCHandler.npcs[i] != null) {
                if (NPCHandler.npcs[i].absX == coordX && NPCHandler.npcs[i].absY == coordY) {
    ...
    public double GetItemShopValue(int ItemID, int Type, int fromSlot) {
        ...
        Overstock = ShopHandler.ShopItemsN[MyShopID][fromSlot] -
                ShopHandler.ShopItemsSN[MyShopID][fromSlot];
        ...
        for (int i = 0; i < ItemHandler.MaxListedItems; i++) {
            if (ItemHandler.ItemList[i] != null) {
                if (ItemHandler.ItemList[i].itemName == NotedName) {
                    if (ItemHandler.ItemList[i].itemDescription.startsWith("Swap this note at any bank for a") == false) {
        ...
        if (playerLevel[playerCooking] >= cooking[1]) {
            ...
            if (actionTimer == 0 && cooking[0] == 2) {
                ...
                if (StopBurnLevel > playerLevel[playerCooking] &&
                    ...
                    addItem(cooking[6], 1);
                    printTo("Msg You burned the " + GetItemName(cooking[5]) + ".");
                } else {
                    addItem(cooking[4], 1);
                    addSkillXP((cooking[2] * cooking[3]), playerCooking);
                    printTo("Msg You cooked the " + GetItemName(cooking[5]) + ".");
                }
                playerEquipment[playerWeapon] = OriginalWeapon;
                playerEquipment[playerShield] = OriginalShield;
                OriginalWeapon = -1;
            ...
            printTo("Msg You need " + cooking[1] + " " + statName[playerCooking] + " to cook this " +
    ...
    public boolean crafting() {
        if (playerLevel[playerCrafting] >= crafting[1]) {
            ...
            if (actionTimer == 0 && crafting[0] == 2) {
                ...
                if (crafting[4] == 1633) {
                    printTo("Msg You crushed the gem.");
                } else {
                    addSkillXP((crafting[2] * crafting[3]), playerCrafting);
                    printTo("Msg You successfully cut the gem.");
                }
                playerEquipment[playerWeapon] = OriginalWeapon;
                playerEquipment[playerShield] = OriginalShield;
                OriginalWeapon = -1;
            ...
            printTo("Msg You need " + crafting[1] + " " + statName[playerCrafting] + " to cut this gem.");
            resetCR();
            return false;
        }
        return true;
        ...
        if (playerLevel[playerFishing] >= fishing[1]) {
            if (freeSlots() > 0) {
                if (fishing[0] == 1) {
                    ...
                    fishing[0] = 2;
                    printTo("Msg You start fishing with your " + GetItemName(fishing[6]) + "...");
                }
                ...
                if (fishing[0] == 2 && misc.random2(Rnd) == 1) {
                    ...
                    if (fishing[5] > -1) {
                        if (IsItemInBag(fishing[5]) == false) {
                            DoFish = false;
                            printTo("Msg You have run out of " + GetItemName(fishing[5]) + ".");
                            playerEquipment[playerWeapon] = OriginalWeapon;
                            playerEquipment[playerShield] = OriginalShield;
                            OriginalWeapon = -1;
                            OriginalShield = -1;
                    ...
                    if (DoFish == true) {
                        ...
                        printTo("Msg You catch a " + GetItemName(fishing[4]) + ".");
                    }
                }
            } else {
                printTo("Msg Not enough space in your inventory.");
            ...
            printTo("Msg You need " + fishing[1] + " " + statName[playerFishing] + " to fish here.");
            resetFI();
            return false;
        }
        return true;
        ...
        switch (useitems[1]) {
            ...
            case 1521:
                if (fletching[6] == 1 && fletching[5] == 2) {
                    printTo("Msg You cannot fletch Arrow Shaft from these logs.");
                    resetFL();
                    break;
                }
            case 1511:
                    ...
                    switch (fletching[6]) {
                        ...
                        case 2:
                            if (useitems[1] == 1511) {
                                ...
                                if (flag == false) {
                                    printTo("Msg To make an ogre composite bow you must have wolf bones in your inventory.");
                                    resetFL();
                                    return false;
                                }
                                fletching[1] = 30;
        ...
        if (freeSlots() > 0) {
            switch (useitems[1]) {
                ...
                case 1539:
                    if (IsItemInBag(2347) == false) {
                        printTo("Msg To make a steel brutal arrows you must have a hammer in your inventory.");
                        resetFL();
                        return false;
                    }
                    Status = 7;
                ...
                case 4819:
                    if (IsItemInBag(2347) == false) {
                        printTo("Msg To make a bronze brutal arrows you must have a hammer in your inventory.");
                        resetFL();
                        return false;
                    }
                    Status = 7;
                ...
                case 4820:
                    if (IsItemInBag(2347) == false) {
                        printTo("Msg To make a iron brutal arrows you must have a hammer in your inventory.");
                        resetFL();
                        return false;
                    }
                    Status = 7;
                ...
                case 4821:
                    if (IsItemInBag(2347) == false) {
                        printTo("Msg To make a black brutal arrows you must have a hammer in your inventory.");
                        resetFL();
                        return false;
                    }
                    Status = 7;
                ...
                case 4822:
                    if (IsItemInBag(2347) == false) {
                        printTo("Msg To make a mithril brutal arrows you must have a hammer in your inventory.");
                        resetFL();
                        return false;
                    }
                    Status = 7;
                ...
                case 4823:
                    if (IsItemInBag(2347) == false) {
                        printTo("Msg To make a adamant brutal arrows you must have a hammer in your inventory.");
                        resetFL();
                        return false;
                    }
                    Status = 7;
                ...
                case 4824:
                    if (IsItemInBag(2347) == false) {
                        printTo("Msg To make a rune brutal arrows you must have a hammer in your inventory.");
                        resetFL();
                        return false;
                    }
                    Status = 7;
            ...
            if (useitems[1] == 2864) {
                if (playerItemsN[useitems[3]] < Amount1 || playerItemsN[useitems[2]] < Amount2) {
                    ...
                    printTo("Msg Not enough feathers or ogre arrow shafts.");
                }
            } else {
                if (playerItemsN[useitems[3]] < Amount1 || playerItemsN[useitems[2]] < Amount2) {
    ...
    public boolean fletching() {
        if (playerLevel[playerFletching] >= fletching[1]) {
            ...
            if (actionTimer == 0 && fletching[0] == 2) {
                ...
                switch (fletching[6]) {
                    case 1:
                        ...
                        printTo("Msg You carefully cut the wood into 15 arrow shafts.");
                        break;
                    case 2:
                        addItem(fletching[4], 1);
                        printTo("Msg You carefully cut the wood into an unstrung shortbow.");
                        break;
                    case 3:
                        addItem(fletching[4], 1);
                        printTo("Msg You carefully cut the wood into an unstrung longbow.");
                        break;
                    case 4:
                        addItem(fletching[4], 1);
                        printTo("Msg You carefully cut the wood into a dramen staff.");
                        break;
                    case 5:
                        int Amount = (misc.random(5) + 1);
                        fletching[2] *= Amount;
                        addItem(fletching[4], Amount);
                        printTo("Msg You carefully cut the wood into " + Amount + " ogre arrow shafts.");
                        break;
                    case 6:
                        addItem(fletching[4], 1);
                        for (int i = 0; i < playerItems.length; i++) {
                        ...
                        printTo("Msg You carefully cut the wood into an unstrung Ogre Composite Bow.");
                        break;
                }
            ...
            printTo("Msg You need " + fletching[1] + " " + statName[playerFletching] + " to fletch those logs.");
            resetFL();
            return false;
        }
        return true;
    ...
    public boolean fletching2(int Status, int Amount1, int Amount2) {
        if (playerLevel[playerFletching] >= fletching[1]) {
            ...
            if (Status == 1) {
                printTo("Msg You attach feathers to " + Amount2 + " arrow shafts.");
            } else if (Status == 2) {
                printTo("Msg You attach some of the heads to some of your headless arrows.");
                printTo("Msg You finish making " + Amount2 + " arrows.");
            } else if (Status == 3) {
                printTo("Msg You add a string to the bow.");
            } else if (Status == 4) {
                printTo("Msg You finish making " + Amount2 + " darts.");
            } else if (Status == 5) {
                printTo("Msg You attach feathers to " + Amount2 + " ogre arrow shafts.");
            } else if (Status == 6) {
                printTo("Msg You attach some of the wolf bone arrow heads to some of your flighted ogre arrows.");
                printTo("Msg You finish making " + Amount2 + " ogre arrows.");
            } else if (Status == 7) {
                printTo("Msg You hammer the nails and attach some to some of your flighted ogre arrows.");
                printTo("Msg You finish making " + Amount2 + " brutal arrows.");
            }
        } else {
            printTo("Msg You need " + fletching[1] + " " + statName[playerFishing] + " to make this.");
            resetFL();
            return false;
        }
        resetFL();
    ...
    public boolean healing() {
        ...
        if (actionTimer == 0 && healing[0] == 2) {
            ...
            printTo("Msg You eat the " + GetItemName(healing[4]) + ".");
            playerEquipment[playerWeapon] = OriginalWeapon;
            playerEquipment[playerShield] = OriginalShield;
            OriginalWeapon = -1;
            OriginalShield = -1;
            ...
            if (playerLevel[playerMining] >= mining[1]) {
                if (freeSlots() > 0) {
                    if (actionTimer == 0 && IsMining == false) {
                        actionAmount++;
                        printTo("Msg You swing your pick axe at the rock...");
                    ...
                    if (actionTimer == 0 && IsMining == true) {
                        ...
                        if (misc.random(RndGems) == (int) (RndGems / 4)) {
                            ...
                            printTo("Msg You get a gem!");
                        } else {
                            addSkillXP((mining[2] * mining[3]), playerMining);
                            addItem(mining[4], 1);
                            printTo("Msg You get some ores.");
                        }
                        playerEquipment[playerWeapon] = OriginalWeapon;
                        OriginalWeapon = -1;
                        playerEquipment[playerShield] = OriginalShield;
                ...
                printTo("Msg You need " + mining[1] + " " + statName[playerMining] + " to mine those ores.");
                resetMI();
                return false;
            }
        } else {
            printTo("Msg You need a pick axe to mine ores.");
            resetMI();
            return false;
        }
        return true;
    ...
    public boolean prayer() {
        if (playerLevel[playerPrayer] >= prayer[1]) {
            ...
            if (actionTimer == 0 && prayer[0] == 2) {
                ...
                printTo("Msg You bury the bones.");
                playerEquipment[playerWeapon] = OriginalWeapon;
                playerEquipment[playerShield] = OriginalShield;
                OriginalWeapon = -1;
                OriginalShield = -1;
            ...
            printTo("Msg You need " + prayer[1] + " " + statName[playerPrayer] + " to bury these bones.");
            resetPR();
            return false;
        }
        return true;
    ...
    public boolean smelting() {
        ...
        if (playerLevel[playerSmithing] >= smelting[1]) {
            if (actionTimer == 0 && smelting[0] == 1) {
                actionAmount++;
                printTo("Msg You place the ores in the furnace...");
                actionTimer = 8;
                if (actionTimer < 1) {
                    actionTimer = 1;
                }
            ...
            printTo("Msg You need " + smelting[1] + " " + statName[playerSmithing] + " to smelt those ores.");
            resetSmelt();
            return false;
        }
        return true;
    ...
    public int CheckSmithing(int ItemID, int ItemSlot) {
        ...
        int Type = -1;
        if (IsItemInBag(2347) == false) {
            printTo("Msg You need a " + GetItemName(2347) + " to hammer bars.");
            return -1;
        }
        switch (ItemID) {
            ...
            case 2363: //Runite Bar
                ...
                break;
            default:
                printTo("Msg You cannot smith this item.");
                GoFalse = true;
                break;
        }
        if (GoFalse == true) {
                ...
                if (AreXItemsInBag(barid, bars) == true) {
                    if (freeSlots() > 0) {
                        if (actionTimer == 0 && smithing[0] == 1) {
                            ...
                            OriginalShield = playerEquipment[playerShield];
                            playerEquipment[playerShield] = -1;
                            printTo("Msg You start hammering the bar...");
                            actionTimer = 7;
                            setAnimation(0x382);
                            smithing[0] = 2;
                        }
                        if (actionTimer == 0 && smithing[0] == 2) {
                            ...
                            printTo("Msg You smith a " + GetItemName(smithing[4]) + ".");
                            resetAnimation();
                            if (smithing[5] <= 1) {
                                resetSM();
                            } else {
                    ...
                    printTo("Msg You need " + bars + " " + GetItemName(barid) + " to smith a " + GetItemName(smithing[4]));
                    resetAnimation();
                    resetSM();
                }
            } else {
                printTo("Msg You need " + smithing[1] + " " + statName[playerSmithing] + " to smith a " +
            ...
            if (playerLevel[playerWoodcutting] >= woodcutting[1]) {
                if (freeSlots() > 0) {
                    if (actionTimer == 0 && IsCutting == false) {
                        actionAmount++;
                        printTo("Msg You swing your axe at the tree...");
                ...
                printTo("Msg You need " + woodcutting[1] + " " + statName[playerWoodcutting] + " to cut those logs.");
                resetWC();
                return false;
            }
        } else {
            printTo("Msg You need an Axe to cut logs.");
            resetWC();
            return false;
        }
        return true;
    ...
    public boolean tradeItem(int itemID, int fromSlot, int amount) {
        if (tradeWith > 0) {
            if (PlayerHandler.players[tradeWith] == null) {
                DeclineTrade();
                printTo("Msg FORCED DECLINE BY SERVER !");
                return false;
            }
        } else {
            DeclineTrade();
    ...
    public boolean sellItem(int itemID, int fromSlot, int amount) {
        if (amount > 0 && itemID == (playerItems[fromSlot] - 1)) {
            if (ShopHandler.ShopSModifier[MyShopID] > 1) {
                ...
                if (IsIn == false) {
                    printTo("Msg You cannot sell " + GetItemName(itemID) + " in this store.");
                    return false;
                }
            }
            if (Item.itemSellable[(playerItems[fromSlot] - 1)] == false) {
                printTo("Msg I cannot sell " + GetItemName(itemID) + ".");
                return false;
            }
            if (amount > playerItemsN[fromSlot] &&
    ...
    public boolean buyItem(int itemID, int fromSlot, int amount) {
        if (amount > 0 && itemID == (ShopHandler.ShopItems[MyShopID][fromSlot] - 1)) {
            ...
            for (int i = amount; i > 0; i--) {
                ...
                if (Slot == -1) {
                    printTo("Msg You don't have enough coins.");
                    break;
                }
