    ...
    implements Runnable
{
    ...
    {
            String storeDirectoryName = ".file_store_32";
            String cacheRoots[] = {
                "./cache/",
                "./moparscape-reference/server/moparscape/cache/",
                "./server/moparscape/cache/",
                "../cache/",
                "../moparscape-reference/server/moparscape/cache/",
                "../server/moparscape/cache/"
            };
            for(int index = 0; index < cacheRoots.length; index++)
            {
                try
                {
                    File storeDirectory = new File(cacheRoots[index] + storeDirectoryName);
                    if(storeDirectory.exists())
                        return storeDirectory.getPath() + "/";
                    File parentDirectory = storeDirectory.getParentFile();
                    if(parentDirectory != null && parentDirectory.exists() && storeDirectory.mkdir())
                        return storeDirectory.getPath() + "/";
                }
                catch(Exception _ex) { }
            }
